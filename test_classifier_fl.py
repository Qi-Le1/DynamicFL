...
def runExperiment():
    ...
    result = resume(cfg['model_tag'], load_tag='checkpoint')
    train_logger = result['logger'] if 'logger' in result else None
    # print('train_logger', train_logger.history, flush=True)
    import numpy as np
    ...
    best_test_acc = max(train_logger.history['test/Accuracy'])
    print(f"{cfg['model_tag']}_best_test_acc: {best_test_acc}", flush=True)
