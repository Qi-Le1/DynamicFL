...
class ClientSelector():
    ...
    def genetic(self, client_ids, num_clients):
        ...
        self.temp_genetic_client_ids = list(client_ids)

        np.random.shuffle(self.temp_genetic_client_ids)
        ...
        for i in range(len(best_x)):
            item = best_x[i]
            if int(item) == 1:
                res.append(self.temp_genetic_client_ids[i])
    ...
    def select_way(self, client_ids, num_clients):
        ...
        if cfg['select_way'] == 'dyna':
            ...
            selected_client_ids = np.random.choice(temp_client_ids, size=num_clients, replace=False)
        else:
    ...
    def select_clients(self, local_gradient_update, prev_selected_client_ids, clients):
        ...
        if local_gradient_update % cfg['high_freq_interval'] == 0:
            selected_client_ids.extend(list((set(prev_selected_client_ids) - set(self.previous_high_freq_clients)) | set(self.current_high_freq_clients)))
        ...
        if local_gradient_update % cfg['low_freq_interval'] == 0:
            selected_client_ids.extend(list((set(prev_selected_client_ids) - set(self.previous_low_freq_clients)) | set(self.current_low_freq_clients)))
            if cfg['client_ratio'] == '1-0':
    ...
    def preprocess_for_client_selection(self):
        ...
        if cfg['client_ratio'] == '1-0':
            ...
            # for local_gradient_update in range(3):
