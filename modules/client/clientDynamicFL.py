...
class ClientDynamicFL(ClientBase):
    ...
    def train(
        ...
        while cur_grad_updates_num <= grad_updates_num:
            ...
            #     dataset={'train': copy.deepcopy(dataset)}, 
            #     tag='client',
            # )['train']
            ...
            for i, input in enumerate(data_loader): 
                ...
                if cfg['grad_clip_way'] == 'adap':
                    if grad_updates_num > 50:
                        # torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=cfg['max_clip_norm'] * math.sqrt(grad_updates_num/(grad_updates_num-1)))
                        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=cfg['max_clip_norm'])

                elif cfg['grad_clip_way'] == 'noadap':
                    if cfg['max_clip_norm'] != 999999:
                        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=cfg['max_clip_norm'])
                optimizer.step()
        ...
        self.optimizer_state_dict = optimizer.state_dict()
        self.model_state_dict = {k: v.cpu() for k, v in model.state_dict().items()}
        # print(f'train_model.state_dict(): {self.model_state_dict}')
        return
...
class Communication:
    ...
    def calculate_local_gradient_update_list(
        ...
        # if cfg['cut_low_freq'] == True:
        #     # longer local gradient update list means higher freq
        #     cls.cut_low_frequency(
        #         local_gradient_update_list_to_client_ratio[0][0],
