...
class ServerFedDyn(ServerBase):
    ...
    def __init__(
        ...
        # self.hist_params_diffs = np.zeros((n_clnt, n_par)).astype('float32')
                ...
                for k, v in model.named_parameters():
                    ...
                    if 'weight' in parameter_type or 'bias' in parameter_type:
                        ...
                        for m in range(len(valid_clients)):
                            ...
                            # self.feddyn_alpha_adaptive[valid_clients_index[m]] *
                            ...
                            # update hist
                            # print(torch.tensor(self.feddyn_alpha_adaptive[valid_clients_index[m]]).is_cuda)
                            # print(torch.tensor(1/cfg['num_clients']).is_cuda)
                            # print((feddyn_tmp_v - self.server_model_state_dict[k]).is_cuda)
                            # print(self.hist[k].is_cuda)
                        self.hist[k] -= feddyn_tmp_v 
                            # self.hist[k] - feddyn_tmp_v + torch.tensor(self.feddyn_alpha_adaptive[valid_clients_index[m]]) * \
                            # torch.tensor(1/cfg['num_clients']) * (feddyn_tmp_v - self.server_model_state_dict[k])
                        # will flip sign in updating gradient

                        # torch.tensor(1/cfg['feddyn_alpha']) *
